...
- **内存**：已用/总量显示，百分比进度条，底部显示占用内存最大的 3 个进程
- **存储**：已用/总量显示
- **网络**：实时下载/上传速率，Canvas 60fps 贝塞尔曲线图表，180 个采样点，左上角显示 IP
...
- **版本信息**：显示本地 git commit hash 和时间，同时查询远程仓库最新版本，对比提示是否可更新
- **端口配置**：可自定义 APPanel 的 Flask/WebSocket 端口和 AP 后端检测端口，保存后重启生效
...
### 其他特性
- **熄屏挂机**：一键熄灭手机屏幕（OLED 像素完全关闭），APP 保持运行不锁频，不影响音乐/下载
- WebSocket 实时推送（0.1s 间隔）
- 标题点击可自定义修改
- 电池高温保护（50°C 持续 30 分钟自动杀 AP 和碧蓝航线）
- AP 后端存活检测（TCP 端口检测 127.0.0.1:22267，端口可自定义）
- 无 AP 环境自动隐藏 AP 相关按钮
- 自定义浮动滚动条（水滴交互效果）
- 禁止文本选中，防止误拖选
- 主题实时切换，无需刷新页面
...
```bash
# 直接在 Termux 家目录克隆（--depth 1 只拉取最新版本，节省空间）
git clone --depth 1 https://github.com/YuCould/APPanel.git ~/APPanel
cd ~/APPanel
python dashboard.py
```
